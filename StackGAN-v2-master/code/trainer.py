...
#######################################################################################
    ...
    def compute_ssim(self, fake_img, real_img):
        fake_img = fake_img.permute(0, 2, 3, 1)
        real_img = real_img.permute(0, 2, 3, 1)
...
#######################################################################################
        # Perception Loss
        # perception_loss = self.l1_loss(self.loss_network(fake_imgs), self.loss_network(real_imgs))
# #########################################################################################
        # real_logits[0]：包含c向量的;real_logits[1]：不包含c向量的
        errD_real = criterion(real_logits[0], real_labels)  # 求解loss
        errD_fake = criterion(fake_logits[0], fake_labels)
        #
        # errD = errD_real + errD_fake + 0.001 * perception_loss
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                ssim_scores = self.compute_ssim(self.fake_imgs[-1].detach(), self.real_imgs[-1])
        ...
        if cfg.TRAIN.NET_G == '':
            ...
            for step in xrange(num_batches):
                ...
                if cfg.TEST.B_EXAMPLE:
                    ...
                    self.save_superimages(fake_imgs[-1], folder, cnt, 128)
                else:
                    # self.save_singleimages(fake_imgs[-1], folder, cnt, 256)
                    self.save_singleimages(fake_imgs[-1], folder, cnt, 128)
...
#######################################################################################
    ...
    def compute_ssim_real(self, real_img0, real_img1):
        img0 = real_img0.permute(0, 2, 3, 1)
        img1 = real_img1.permute(0, 2, 3, 1)
        img0 = img0.cpu().numpy()
        img1 = img1.cpu().numpy()
        SSIM = ssim(img0, img1, multichannel=True)
        return SSIM

    def compute_ssim_fake(self, fake_img0, fake_img1):
        img0 = fake_img0().permute(0, 2, 3, 1)
        img1 = fake_img1().permute(0, 2, 3, 1)
        img0 = img0.cpu().numpy()
        img1 = img1.cpu().numpy()
        SSIM = ssim(img0, img1, multichannel=True)
        return SSIM
...
#########################################################################################
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):   #enumerate()函数将一遍历对象变成索引序列，同时列出数据和数据下标
                ...
                # print('fake_imgs[-1].shape:', self.fake_imgs[-1].shape)     # (8,3,128,128)
                # print('fake_imgs[-2].shape:', self.fake_imgs[-2].shape)     # (8,3,64,64)
                print('real_img[-1].shape:', self.real_imgs[-1].shape)      # (8,3,128,128)
                print('real_img[-2].shape:', self.real_imgs[-2].shape)      # (8,3,64,64)


                ssim_real_scores = self.compute_ssim_real(self.real_imgs[-1], self.wrong_imgs[-1])
                print('ssim_real_scores:', ssim_real_scores)
                ssim_fake_scores = self.compute_ssim_fake(self.fake_imgs[0].detach, self.fake_imgs[0].detach)
                print('ssim_fake_scores:', ssim_fake_scores)
                ...
                if count % 100 == 0:
                    # summary_ssim_real = summary.scalar('ssim_real_score', ssim_real_scores)
                    # summary_ssim_fake = summary.scalar('ssim_fake_score', ssim_fake_scores)
                    ...
                    # self.summary_writer.add_summary(summary_ssim_real, count)
                    # self.summary_writer.add_summary(summary_ssim_fake, count)
    ...
    def evaluate(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            print(netG)
            # state_dict = torch.load(cfg.TRAIN.NET_G)
            print('split_dir:', split_dir)  # valid
            state_dict = \
                torch.load(cfg.TRAIN.NET_G,
                           map_location=lambda storage, loc: storage)   # 在CPU上加载预先训练好的GPU模型,用这种方式
            # state_dict = torch.load(cfg.TRAIN.NET_G)
            ...
            netG.load_state_dict(state_dict)    # 加载模型
            print('Load ', cfg.TRAIN.NET_G)
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                embedding_dim = t_embeddings.size(1)    # 10,一个图片10个文本描述语句
