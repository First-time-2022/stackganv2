...
#打印当前工作路径+文件名/mnt/JetBrainsPyCharm/Deep-Learning-21-Examples-master/StackGAN-v2-master/code/datasets.pyc
...
def get_imgs(img_path, imsize, bbox=None,
    ...
    # img_path = img_path[0:22] + img_path[34:]   # img_path: ../data/flowers/images/class_00007/image_07212.jpg去掉 class
    # print('img_path_gai:', img_path)
...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train', embedding_type='cnn-rnn',
                 base_size=32, transform=None, target_transform=None):  ################### base_size 从64改成32
        self.transform = transform
        self.norm = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        ...
        self.imsize = []      ###################################### 3个变4个，加了第一个值16
        for i in range(cfg.TREE.BRANCH_NUM):  #####################  循环3次，加上64，128，256
            self.imsize.append(base_size)
            base_size = base_size * 2
        print('self.imsize:', self.imsize)  #[16,64,128,258]没错误
...
#######################################################################flowers数据集
        # path = os.path.join(self.data_dir + '/' + 'train')  # 需自定义路径
        # txt_name = path + '/' + 'images' + '.txt'
        # print(txt_name)
        #
        # caption_dict = {}
        # print(self.data_dir)
        # root_dir = os.path.join(self.data_dir + '/text_c10')
        # print(root_dir)
        # list = os.listdir(root_dir)  # 列出text_c10下的所有class_id文件夹
        # print(list)
        # print('length:',len(list))
        # print(self.filenames)
        # print('filenames:',len(self.filenames))
        #
        # print('class_id:',len(self.class_id))
        #
        # with open(txt_name, "w") as f:
        #     for i in range(0, len(list)):
        #         path0 = os.path.join(root_dir + '/' + list[i])
        #         print(path0) # ../data/flowers/text_c10/class_00034
        #         path1 = os.listdir(path0) # 列出class_id里的txt文件
        #         print(path1)# class_id里面的所有txt文件
        #
        #         for a in range(0, len(path1)):
        #             path2 = os.path.join(path0 + '/'+path1[a]) # 路径名拼接.../class_id/image_xx.txt
        #             path3 = path2[-27:] #截取class_id/image_xx.txt
        #             print(path3)
        #
        #             # f.write(path3[12:-4])  # 去掉.txt和class_id,并存入images.txt文件中
        #             # f.write('\r\n')  # 逐行写入
        #
        #             caption_name = path2
        #             captions = load_captions(caption_name) #加载txt文件里的文本内容
        #             caption_dict[a] = captions
        #
        #         print(path2)
        #
        ...
        # # file1.close()
    # ##################################################################################
        return caption_dict # load_all_captions()的返回值
...
# --------------------------------------------------------------------------------------------------#
    ...
    def load_embedding(self, data_dir, embedding_type):
        ...
        with open(data_dir + embedding_filename, 'rb') as f:
            ...
            print('embeddings: ', embeddings.shape) # flowers:(7034,10,1024)
        return embeddings
    ...
    def load_filenames(self, data_dir):
        ...
        # filepath = os.path.join(data_dir, 'filenames2.pickle')  # 训练7043张flowers
        filepath = os.path.join(data_dir, 'filenames_8855.pickle')    # 训练 8855张birds
        with open(filepath, 'rb') as f:
            filenames = pickle.load(f)
        print('Load filenames from: %s (%d)' % (filepath, len(filenames)))
        return filenames

    def prepair_training_pairs(self, index):    # 预配对
