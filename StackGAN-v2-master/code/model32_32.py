...
class INIT_STAGE_G1(nn.Module):     # 仅包含一个上采样层，用以32*32变为64*64
    def __init__(self, ngf):
    ...
    def define_module(self):
        ...
        # self.upsample1 = upBlock(ngf, ngf // 2) # 通道数64->64
        # self.upsample2 = upBlock(ngf // 2, ngf // 4)    # 32->32
        # self.upsample3 = upBlock(ngf, ngf // 2)    # 16->16
        self.upsample4 = upBlock(ngf, ngf // 2)   # 8->8
...
class NEXT_STAGE_G0(nn.Module):
    def __init__(self, ngf, num_residual=cfg.GAN.R_NUM):
        super(NEXT_STAGE_G0, self).__init__()
        self.gf_dim = ngf
        if cfg.GAN.B_CONDITION:
            self.ef_dim = cfg.GAN.EMBEDDING_DIM
        else:
    ...
    def forward(self, h_code, c_code):  # 注意，2D卷积层的输入data维数是 batchsize*channel*height*width
...
class G_NET(nn.Module):
    ...
    def define_module(self):
        ...
        if cfg.TREE.BRANCH_NUM > 0:
            ...
            self.h_net1 = INIT_STAGE_G(self.gf_dim * 16)
            self.img_net1 = GET_IMAGE_G(self.gf_dim * 2)    # 生成32*32*3的图片
        if cfg.TREE.BRANCH_NUM > 1:
            self.h_net2 = NEXT_STAGE_G0(self.gf_dim * 2)
            self.img_net2 = GET_IMAGE_G(self.gf_dim)    # 生成64*64*3的图片
        if cfg.TREE.BRANCH_NUM > 2:
            self.h_net3 = NEXT_STAGE_G(self.gf_dim)
            self.img_net3 = GET_IMAGE_G(self.gf_dim // 2)   # 生成128*128*3的图片
        if cfg.TREE.BRANCH_NUM > 3: # Recommended structure (mainly limited by GPU memory), and not test yet
            ...
            self.h_net4 = NEXT_STAGE_G(self.gf_dim // 2)
            self.img_net4 = GET_IMAGE_G(self.gf_dim // 4)   # 生成256*256*3的图片
        if cfg.TREE.BRANCH_NUM > 4:
            # self.h_net4 = NEXT_STAGE_G(self.gf_dim // 8, num_residual=1)
            # self.img_net4 = GET_IMAGE_G(self.gf_dim // 16)
            self.h_net5 = NEXT_STAGE_G(self.gf_dim // 4, num_residual=1)
            self.img_net5 = GET_IMAGE_G(self.gf_dim // 18)
    ...
    def forward(self, z_code, text_embedding=None):
        ...
        if cfg.TREE.BRANCH_NUM > 0:
            ...
            # print('h_code1.shape:', h_code1.shape)
            fake_img1 = self.img_net1(h_code1)
            # print('fake_img1.shape:', fake_img1.shape)
            fake_imgs.append(fake_img1)
        if cfg.TREE.BRANCH_NUM > 1:
            # h_code2 = self.h_net2(h_code1, c_code)
            h_code2 = self.h_net2(h_code1, c_code)
            # print('h_code2.shape:', h_code2.shape)
            fake_img2 = self.img_net2(h_code2)
            # print('fake_img2.shape:', fake_img2.shape)
            fake_imgs.append(fake_img2)
        if cfg.TREE.BRANCH_NUM > 2:
            h_code3 = self.h_net3(h_code2, c_code)
            # print('h_code3.shape:', h_code3.shape)
            fake_img3 = self.img_net3(h_code3)
            # print('fake_img3.shape:', fake_img3.shape)
            fake_imgs.append(fake_img3)
        if cfg.TREE.BRANCH_NUM > 3:
            h_code4 = self.h_net4(h_code3, c_code)
            # print('h_code4.shape:', h_code4.shape)
            fake_img4 = self.img_net4(h_code4)
            # print('fake_img4.shape:', fake_img4.shape)
            fake_imgs.append(fake_img4)
        if cfg.TREE.BRANCH_NUM > 4:
            h_code5 = self.h_net4(h_code4, c_code)
            # print('h_code5.shape:', h_code5.shape)
            fake_img5 = self.img_net5(h_code5)
            # print('fake_img5.shape:', fake_img5.shape)
            fake_imgs.append(fake_img5)

        # list = np.array(fake_imgs)
        # print('fake_img_all.shape:', list.shape)
        # print('fake_img1.shape:', list[0].shape)
        # print('fake_img2.shape:', list[1].shape)
        # print('fake_img3.shape:', list[2].shape)
        # print('fake_img4.shape:', list[3].shape)
...
def encode_image_by_16times(ndf):
    encode_img = nn.Sequential(
        ...
        nn.Conv2d(ndf * 2, ndf * 4, 4, 2, 1, bias=False),   # Conv2d(in_channels, out_channels, kernel_size, stride, padding)
...
# Downsale the spatial size by a factor of 4   # 空间尺寸缩小4倍  为了32*32*3 变成 4*4*8ndf
def encode_image_by_8times(ndf):
    encode_img = nn.Sequential(
        # --> state size. ndf x in_size/2 x in_size/2   16*16*3->8*8*ndf
        nn.Conv2d(3, ndf, 4, 2, 1, bias=False),
        nn.LeakyReLU(0.2, inplace=True),
        ...
        nn.BatchNorm2d(ndf * 8),
        nn.LeakyReLU(0.2, inplace=True),
        # # --> state size 8ndf x in_size/16 x in_size/16    ndf*8=512
        # nn.Conv2d(ndf * 4, ndf * 8, 4, 2, 1, bias=False),
        # nn.BatchNorm2d(ndf * 8),
        # nn.LeakyReLU(0.2, inplace=True)
    )
...
class D_NET32(nn.Module):
    ...
    def define_module(self):
        ndf = self.df_dim
        efg = self.ef_dim
        self.img_code_s8 = encode_image_by_8times(ndf)    # 32*32*3变成4*4*8ndf
        ...
        if cfg.GAN.B_CONDITION:
            self.jointConv = Block3x3_leakRelu(ndf * 8 + efg, ndf * 8)  # 通道数由ndf * 8 + efg变成ndf*8，长宽不变
    ...
    def forward(self, x_var, c_code=None):
        ...
        # print('x_var16.shape:', x_var.shape)
        # print('x_code16_shape:', x_code.shape)
...
class D_NET64(nn.Module):
    ...
    def define_module(self):
        ndf = self.df_dim
        efg = self.ef_dim
        self.img_code_s16 = encode_image_by_16times(ndf)    # 通道数8ndf，长宽缩小16倍 4*4*8ndf
...
# For 64 x 64 images
class D_NET64_gai(nn.Module):
    def __init__(self):
        super(D_NET64_gai, self).__init__()
        self.df_dim = cfg.GAN.DF_DIM    # config.py中，64
        self.ef_dim = cfg.GAN.EMBEDDING_DIM
        self.define_module()
    ...
    def forward(self, x_var, c_code=None):
        x_code = self.img_code_s16(x_var)
        # print('x_var64.shape:', x_var.shape)
        # print('x_code64_shape:', x_code.shape)

        if cfg.GAN.B_CONDITION and c_code is not None:
            c_code = c_code.view(-1, self.ef_dim, 1, 1) # 改变shape，-1表示不变，(8,100,1,1)
            c_code = c_code.repeat(1, 1, 4, 4)  # 每个维度重复几次，(8*1,100*1,1*4,1*4)->(8,100,4,4)
            # state size (ngf+egf) x 4 x 4
            h_c_code = torch.cat((c_code, x_code), 1)   # 按维度1拼接，横着拼
            # state size ngf x in_size x in_size
            h_c_code = self.jointConv(h_c_code)
...
class D_NET128(nn.Module):
    ...
    def define_module(self):
        ndf = self.df_dim
        efg = self.ef_dim
        self.img_code_s16 = encode_image_by_16times(ndf)    # 通道数变为8ndf,长、宽各缩小16倍 8*8*8ndf
        self.img_code_s32 = downBlock(ndf * 8, ndf * 16)    # 通道数变为16ndf,长、宽各缩小一半 4*4*16ndf
        self.img_code_s32_1 = Block3x3_leakRelu(ndf * 16, ndf * 8)  # 通道数变为8ndf,长、宽不变 4*4*8ndf
    ...
    def forward(self, x_var, c_code=None):
        ...
        # print('x_var128.shape:', x_var.shape)
        # print('x_code128_shape:', x_code.shape)
...
class D_NET256(nn.Module):
    ...
    def define_module(self):
        ndf = self.df_dim
        efg = self.ef_dim
        self.img_code_s16 = encode_image_by_16times(ndf)    # 16*16*8ndf
        self.img_code_s32 = downBlock(ndf * 8, ndf * 16)    # 8*8*16ndf
        self.img_code_s64 = downBlock(ndf * 16, ndf * 32)   # 4*4*32ndf
        self.img_code_s64_1 = Block3x3_leakRelu(ndf * 32, ndf * 16) # 4*4*16ndf
        self.img_code_s64_2 = Block3x3_leakRelu(ndf * 16, ndf * 8)  # 4*4*8ndf
    ...
    def forward(self, x_var, c_code=None):
        ...
        # print('x_var256_shape:', x_var.shape)
        # print('x_code256.shape:', x_code.shape)
